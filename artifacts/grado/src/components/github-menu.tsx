...
export function GithubMenu({ token, html, conversationId, siteTitle, onImport, compact = true, align = "bottom" }: GithubMenuProps) {
  ...
  useEffect(() => {
    ...
    if (status === "connected" || status === "error") {
      ...
      if (status === "connected" && token) {
        fetch("/api/github/status", { headers: { Authorization: `Bearer ${token}` } })
          .then((r) => r.json())
          .then((d) => { setConnected(!!d.connected); setUsername(d.username || null); updatePos(); setPanel("menu"); })
          .catch(() => {});
      }
    }
  }, [token]);
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      const popup = document.getElementById("github-portal");
  ...
  const updatePos = () => {
    ...
    setPos({
      top: align === "bottom" ? r.bottom + 8 : r.top - 8,
      ...
      {panel && (
        <motion.div
          ...
          style={{
            position: "fixed",
            zIndex: 9999,
            top: align === "bottom" ? pos.top : undefined,
            bottom: align === "top" ? `calc(100vh - ${pos.top}px)` : undefined,
            right: pos.right,
          }}
          className="bg-[#1a1a24] border border-[#2a2a38] rounded-xl p-3 w-80 shadow-xl"
        >
